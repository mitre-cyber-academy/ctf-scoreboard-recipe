class ApplicationController < ActionController::Base
  ...
  def load_game
    ...
    unless current_user.nil?
      now = Time.now
      if now < @game.start
        flash.now[:info] = "The game will open <strong><span id=\"major-tom\"></span><noscript> on #{@game.start.strftime("%b %e %y, %R %Z")}</noscript></strong>."
      elsif now < @game.stop && now > @game.stop - 1.hour
        flash.now[:info] = "The game will stop accepting submissions <strong><span id=\"major-tom\"></span><noscript> on #{@game.stop.strftime("%b %e %y, %R %Z")}</noscript></strong>."
      else
